...
class Stretch4MujocoSimulator(StretchMujocoSimulator):
    ...
    # stretch4_urdf tool name for the parallel gripper (PG4), to pass as `tool_name`.
    PARALLEL_GRIPPER_TOOL_NAME = "eoa_wrist_dw4_tool_pg4"
    ...
    def __init__(
        ...
        """
        `tool_name` picks the stretch4_urdf tool to build the robot with, e.g.
        `PARALLEL_GRIPPER_TOOL_NAME` for the parallel gripper (PG4). None uses the
        standard Stretch 4 gripper (SG4). The `gripper` actuator keeps the same units
        for either gripper, see `config.robot_settings_se4_pg4`.

        Note that a prebuilt `model` already has its tool baked in, so pass the same
        `tool_name` that the model was generated with.
        """
        ...
        # The server only loads it on `start()`.
        self.get_robot_xml_path(urdf_path=self.urdf_path)
        ...
        self.T_base_left = self.urdf_model.get_transform(joint_positions, "lidar_left_link")
        self.T_base_right = self.urdf_model.get_transform(joint_positions, "lidar_right_link")
    ...
    @staticmethod
    def get_robot_xml_path(tool_name: str | None = None, urdf_path: str | None = None) -> str:
        """
        Generates the robot MJCF and returns its XML path.

        Args:
            tool_name: the stretch4_urdf tool to build with, see `get_urdf_path()`.
                Ignored if `urdf_path` is given.
            urdf_path: a URDF from `get_urdf_path()` to generate from, to skip
                generating another one.
        """
        from stretch4_mujoco.models.stretch_4.mjcf_generator import generate_mjcf

        if urdf_path is None:
            urdf_path = Stretch4MujocoSimulator.get_urdf_path(tool_name)
  ...
  <include file="autogenerated_contact.xml"/>
  ...
  <include file="keyframes.xml"/>
    ...
    def get_urdf_path(tool_name: str | None = None) -> str:
        """
        Generates a URDF with `tool_name`, or the default tool if None, and returns its path.
        """
        model_name, batch_name, default_tool_name = (
