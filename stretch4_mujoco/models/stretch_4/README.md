...
3. Replaces the default wheel joints with custom bodies (`left_wheel_link`, `right_wheel_link`, `back_wheel_link`) containing `class="wheel"` and associated wheel rollers to support omnibase motion, and includes `omniwheels.xml`.
4. Encapsulates lidar and camera geoms with dedicated bodies and includes `hemisphere_lidar_cameras_right/left.xml`.
5. Replaces `mast_link` geometry with appropriate `collision` and `visualgeom` classes.
6. Renames the root body to `base_link` and adds an `imu` site.
7. Encapsulates gripper fingers into `gripper_slider_link` (if present, e.g. for sg4 tool).
