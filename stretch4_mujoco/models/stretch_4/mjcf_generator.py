...
# The parallel gripper (PG4) tool's prismatic finger joints, keyed by the finger
# actuator in actuator_sensor.xml that drives them. The standard gripper (SG4)
# names its revolute finger joints "gripper_finger_{left,right}_joint" instead.
PARALLEL_GRIPPER_FINGER_JOINTS = {
    "gripper_left_finger": "finger_left_joint",
    "gripper_right_finger": "finger_right_joint",
}

# contact.xml is written against the SG4 body names. On PG4 the fingers keep the
# URDF's names and the finger/tip pairs are one body each, so the excludes are
# rewritten through this map, and dropped where a body maps to None.
PARALLEL_GRIPPER_BODY_ALIASES = {
    "gripper_slider_link": "pjg_body_link",
    "gripper_finger_left_link": "finger_left_link",
    "gripper_finger_right_link": "finger_right_link",
    "gripper_fingertip_left_link": None,
    "gripper_fingertip_right_link": None,
}
...
def generate_mjcf(urdf_path: str, out_mjcf_path: str=None):
    ...
    for b in worldbody.findall(".//body"):
        ...
        if (
            "lift_link" in name
            or "arm_link" in name
            or "wrist_link" in name
            or "gripper" in name
            or name in ("pjg_body_link", "finger_left_link", "finger_right_link")
        ):
            b.set("gravcomp", "1")
    ...
    for geom in worldbody.findall(".//geom"):
        mesh = geom.get("mesh", "")
        if "gripper_finger" in mesh or "gripper_fingertip" in mesh or mesh.startswith("finger_"):
    ...
    for j in worldbody.findall(".//joint"):
        ...
        elif name in PARALLEL_GRIPPER_FINGER_JOINTS.values():
            j.set("class", "parallel_gripper_finger")
        else:
    ...
    is_parallel_gripper = all(
        worldbody.find(f".//joint[@name='{name}']") is not None
        for name in PARALLEL_GRIPPER_FINGER_JOINTS.values()
    )
    ...
    try:
        ...
        if os.path.exists(actuator_xml_path):
            ...
            for pos in act_root.findall(".//position"):
                if is_parallel_gripper and pos.get("name") in PARALLEL_GRIPPER_FINGER_JOINTS:
                    pos.set("joint", PARALLEL_GRIPPER_FINGER_JOINTS[pos.get("name")])
                    pos.set("class", "parallel_gripper_finger")
    ...
    generate_contact_xml(out_mjcf_path.parent, body_names, is_parallel_gripper)


def generate_contact_xml(model_dir: Path, body_names: set[str], is_parallel_gripper: bool):
    """
    Writes autogenerated_contact.xml from contact.xml, fitting its excludes to the tool.

    MuJoCo refuses to compile an <exclude> that names a body the model lacks, and
    contact.xml's are written against the standard gripper. Pairs and tendons are
    left alone: the wheel pairs reference the scene's floor, not a robot body.
    """
    tree = ET.parse(model_dir / "contact.xml")
    contact = tree.getroot().find("contact")
    if contact is not None:
        seen = set()
        for exclude in contact.findall("exclude"):
            bodies = [exclude.get("body1"), exclude.get("body2")]
            if is_parallel_gripper:
                bodies = [PARALLEL_GRIPPER_BODY_ALIASES.get(b, b) for b in bodies]
            key = frozenset(bodies)
            if None in bodies or not set(bodies) <= body_names or key in seen:
                contact.remove(exclude)
                continue
            seen.add(key)
            exclude.set("body1", bodies[0])
            exclude.set("body2", bodies[1])

    ET.indent(tree.getroot(), space="  ", level=0)
    tree.write(model_dir / "autogenerated_contact.xml", encoding="unicode", xml_declaration=True)
