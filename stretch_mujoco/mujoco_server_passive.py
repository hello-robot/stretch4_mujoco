    ...
    def _run_ui_simulation(self, show_viewer_ui: bool):
        ...
        with self.viewer as viewer:
            ...
            while viewer.is_running() and not self._is_requested_to_stop():
                ...
                elif (start_time - last_warning) > 30.0:  # Only print warning every 30s
                    click.secho(
                        f"WARNING: Passive viewer and camera rendering is below the requested {1/UI_FPS_CAP_RATE}FPS on the last render.",
                        fg="yellow",
                    )
